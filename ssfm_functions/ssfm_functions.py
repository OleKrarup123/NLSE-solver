...
class TimeFreq:
    ...
    Attributes:
        number_of_points (int): Number of time points. Should be a power of 2 for fast FFT computation
...
def get_photon_number(freq_Hz: npt.NDArray[float],
    ...
    photon_number = field_sum/(PLANCKCONST_J_PER_HZ*delta_freq_Hz)
...
class SSFMResult:
    ...
    pulse_matrix ( npt.NDArray[complex] ): Amplitude of pulse at every z-location in fiber.
    spectrum_matrix ( npt.NDArray[complex] ): Spectrum of pulse at every z-location in fiber.
    """
...
def describe_sim_parameters(
    ...
    comparison, which is saved for reference.

    The vertical blue line indicates the total length of this fiber, so
    any bar that extends beyond it represents an effect, which probably
    (but not certainly!) has little impact on the pulse evolution. 

    Similarly, the vertical gray line represents the step size of the
    simulation, so any bar close to or below it represents an effect,
    which is probably badly modelled by the simulation. In this case,
    the step size can be reduced by reducing the fiber length or 
...
def get_noise_PSD(NF_dB: float,
    ...
    """
    Calculates PSD in units of [J/Hz] of noise from an amplifier behaving like an idealized EDFA:
    ...
    bandwidth is assumed to be infinite (i.e. EVERY frequency experiences amplification!)
...
def SSFM(
    ...
    2) Loops over fibers in fiber_link. Gets zsteps for each and runs the SSFM
    3) Stores results of each fiber in a separate SSFMResult and uses pulse at the end as input to next one
...
def plot_first_and_last_pulse(ssfm_result_list: list[SSFMResult],
    ...
    **kwargs : TYPE
    If firstandlastpulsescale=='log' is contained in keyword args, set y-scale to log.
...
def plot_pulse_chirp_2D(ssfm_result_list: list[SSFMResult],
    ...
    **kwargs : TYPE
    If chirpPlotRange=(f_min_Hz,f_max_Hz) is contained in **kwargs, use these values to set color scale.
